...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None):
        ...
        self.se = nn.Sequential(nn.AdaptiveAvgPool3d(1),
                                nn.Conv3d(planes, planes // 16, kernel_size=1),
                                nn.ReLU(),
                                nn.Conv3d(planes // 16, planes, kernel_size=1),
                                nn.Sigmoid())
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None,groups=1,width_per_group=64):
        ...
        self.stride = stride
        self.se = nn.Sequential(nn.AdaptiveAvgPool3d(1),
                                nn.Conv3d(planes * self.expansion,planes * self.expansion//16,kernel_size=1),
                                nn.ReLU(),
                                nn.Conv3d(planes * self.expansion//16,planes * self.expansion,kernel_size=1),
                                nn.Sigmoid())
...
class ResNet():
    def __init__(self,
                 ...
                 shortcut_type='B',
                 widen_factor=1.0,
                 n_classes=300,groups=32,wid_per_group=4):
        super().__init__()
...
def generate_model(model_depth, **kwargs):
    assert model_depth in [5,10, 18, 34, 50, 101, 152, 200]
    if model_depth == 10:
